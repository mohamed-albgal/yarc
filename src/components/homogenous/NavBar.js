...
export default ({transparent}) => {
    ...
    return (
        <div className={`${transparent ? "sm:bg-transparent sm:absolute sm:z-10 sm:top-0" : ""} sticky bg-gray-900 shadow-inner w-screen `}>
            <nav className={` w-screen lg:w-11/12 sm:pb-1 sm:flex sm:items-center sm:justify-between`}>
                ...
                <div style={openMenuBg} className={`${!closed ? 'h-64 relative z-0 top-0 transition-height duration-300 ease-linear' : ' h-0 transition-height duration-300 ease-linear sm:h-auto sm:w-auto '} flex`}>
...
const NavElement = ({linkTo, text, mobileOnly}) => {
    return (
        <Link className={`${mobileOnly && "sm:hidden relative top-0 left-0"} text-center block py-2 sm:hover:underline sm:hover:bg-indigo-200 hover:text-gray-900 font-semibold px-4 text-xl text-white sm:ml-4`} to={linkTo}>
            {text}
        </Link>
    )
